...
public:
	...
	// returns true if pieces can move (if the king is in check it will return the "can do" squares in vector) or false if only the king can move

	std::vector<Move> generate(std::vector<int> Incheck , int piecePlace );
	MovementDirection kingPin(int target); 
	//return the type of can do move (forward / diagonal / sides) or 0 if the piece is not in pin
